...
class AICommandPlanner:
    ...
    def interpret_natural_language(self, user_prompt: str) -> Dict[str, Any]:
            ...
            try:
                ...
                payload = {
                    ...
                    "messages": [
                        ...
                        # we give the User's natural language command here.
                ...
                # We then send POST request to Groq server. Timeout helps prevent waiting forever if the server is unavailable.
                ...
                if response.status_code == 200:
                    ...
                    # we extract the model's JSON string and convert it into a dictionary next
        ...
        # If the API key does not exist or the API call failed, we will use keyword matching instead.
    ...
    def recommend_mode_from_state(self, state: dict) -> str:
        ...
        # Highest priority: If the sensor is already saturated, it will immediately switch to low-noise mode.
    ...
    def validate_and_plan_bias(self, target_gain_mode: str, current_temp_c: float, expected_power_w: float) -> dict:
        ...
        """
        # Mapping mode intent to candidate bias voltages (Lookup table converting operating mode into its corresponding bias voltage)
        mode_bias_map = {
            "low_noise": 3.0,
            "balanced": 5.0,
        ...
        # Pre-execution simulation using physics engine (We simulate detector behaviour BEFORE applying the command)
        ...
        # Saturation and safety evaluation rule (If the simulation predicts saturation, we reject the requested configuration)
        if simulated_state["is_saturated"]:
            # Recalculate to safe lower bias if candidate causes optical saturation (We then reduce bias voltage to a safer operating point)
            safe_bias = 2.0
